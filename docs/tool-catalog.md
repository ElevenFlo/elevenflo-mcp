...
ElevenFlo MCP exposes read-only research tools for Chapter 11 workflows. Use them to discover cases, search dockets and filings, retrieve exact court-document text, discover source/news metadata and snippets, summarize case materials, and inspect filing relationships.
...
4. `read_document_text` or `extract_from_document` — quote exact language.
...
- **Next step:** `read_document_text` for exact language.
...
- **Use when:** you know the document and want the relevant passage in one call.
- **Required inputs:** `document_id` and `query`.
- **Returns:** exact selected chunks plus the chunk IDs used.
...
- **Next step:** `read_document_text` on anchor filings.
